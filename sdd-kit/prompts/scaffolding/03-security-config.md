...
public class SwaggerConfig {
    ...
    public OpenAPI openAPI() {
        return new OpenAPI()
            .info(new Info()
                .title("AmenicSystem API")
                .version("1.0")
                .description("API do sistema de amenicma"))
...
Implemente o handler global conforme a tabela em `sdd-kit/docs/architecture/layer-rules.md`.
...
spring:
  ...
  datasource:
    url: ${DB_URL:jdbc:postgresql://localhost:5432/amenicsystem}
    username: ${DB_USER:amenicuser}
    password: ${DB_PASS:amenicpass}
    hikari:
...
spring:
  datasource:
    url: jdbc:postgresql://postgres:5432/amenicsystem
  data:
    redis:
      host: redis
```
